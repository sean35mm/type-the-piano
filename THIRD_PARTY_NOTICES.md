# Third-Party Notices

This project includes third-party software and media. These notices supplement, and do not replace, the applicable license texts.

## Runtime software

The production bundle includes the following MIT-licensed packages:

| Package | Copyright notice |
| --- | --- |
| `@tonejs/midi` | Copyright © 2016 Yotam Mann |
| `array-flatten` | Copyright (c) 2014 Blake Embrey (hello@blakeembrey.com) |
| `midi-file` | Copyright © 2016 Carter Thaxton |
| `react`, `react-dom`, `scheduler` | Copyright (c) Meta Platforms, Inc. and affiliates. |

The MIT License permits use, copying, modification, distribution, sublicensing, and sale provided the copyright and permission notice are included. The software is provided without warranty. The deployed combined media and runtime software notice, including the full MIT permission and disclaimer text for these components, is at [`public/assets/NOTICE.txt`](public/assets/NOTICE.txt). Package distributions also contain their original license files.

## Bundled media

- Bernd Krueger's MIDI realization of Chopin's Étude Op. 25 No. 1 is licensed under [CC BY-SA 3.0 Germany](https://creativecommons.org/licenses/by-sa/3.0/de/).
- Bernd Krueger's MIDI realization of Beethoven's Bagatelle WoO 59, "Für Elise," is licensed under [CC BY-SA 3.0 Germany](https://creativecommons.org/licenses/by-sa/3.0/de/).
- Bernd Krueger's MIDI realization of Debussy's "Clair de lune" is licensed under [CC BY-SA 3.0 Germany](https://creativecommons.org/licenses/by-sa/3.0/de/).
- Alexander Holm's Salamander Grand Piano V3 samples are licensed under [CC BY 3.0 Unported](https://creativecommons.org/licenses/by/3.0/).

See [ASSET_ATTRIBUTIONS.md](ASSET_ATTRIBUTIONS.md) for authorship, sources, modifications, and redistribution details. The app's Credits link opens the combined deployed notice at `public/assets/NOTICE.txt`.
